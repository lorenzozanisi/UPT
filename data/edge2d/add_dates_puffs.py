...
def get_puffs(df):
    ...
    df = df[
        ~df.apply(
            lambda row: row["main_ion_puff_species"] in row["impurity_puff_species"] if isinstance(row["impurity_puff_species"], np.ndarray) else False,
            axis=1,
        )
    ]
    ...
    impurities = np.hstack(impurities) 
    impurities = set(np.unique([i for i in impurities])) # set along gives only nans for some reason
    main_ion = set(np.unique([s for s in df["main_ion_puff_species"]]))
    ...
    all_puff_species_decoded = [s for s in all_puff_species_decoded if s!='nan']
    ...
    for species, species_str in zip(all_puff_species, all_puff_species_decoded):
        df[f"{species_str}_puff_present"] = df.apply(
            lambda row: species in row["main_ion_puff_species"]
            or species in row["impurity_puff_species"],
            axis=1,
        )
        df[f"{species_str}_puff_present"] = df[f"{species_str}_puff_present"].astype(
            int
        )
    ...
    def extract_values(row, species):
        if species in row["main_ion_puff_species"]:
            return row["main_ion_puff_values"]
    ...
    for species, species_str in zip(all_puff_species, all_puff_species_decoded):
        df[f"{species_str}_puff_values"] = df.apply(
            lambda row: extract_values(row, species), axis=1
        )
    ...
    for species, species_str in zip(all_puff_species, all_puff_species_decoded):
        df[f"{species_str}_puff_present"] = df.apply(
            lambda row: (
                row[f"{species_str}_puff_present"]
                if row[f"{species_str}_puff_values"] != 0
                else 0
            ),
            axis=1,
        )
    ...
    df = df[
        ~df.apply(
            lambda row: all(
                row[f"{species_str}_puff_values"] == 0
                for species_str in all_puff_species_decoded
            ),
            axis=1,
        )
    ]
